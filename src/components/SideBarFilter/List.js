...
class FilterList extends Component {
  ...
  render() {
    const {categories, roles, url} = this.props

    return (
      <div style={{'paddingBottom': '5px'}}>
        <FilterOption title='Filter options' options={{AND: '', OR: '', ANDOR: ''}} url={url} />
        {categories && Object.entries(categories).map((e,i) => <FilterCard key={i} title={e[0]} options={e[1]} filterType='categories' url={url} />)}
        {roles && Object.entries(roles).map((e,i) => <FilterCard key={i} title={e[0]} options={e[1]} filterType='roles' url={url} />)}
